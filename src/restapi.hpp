...
class GetRequest{
    ...
    protected:
        ...
        void update_rtt(const std::chrono::system_clock::time_point& t1, const std::chrono::system_clock::time_point& t2){
    ...
    public:
        GetRequest(std::string endpoint, std::string url = "api.coinbase.com", std::string protocol = "https") : m_curl(curl_easy_init()){
            std::string uri = protocol + "://" + url + "/api/v3/brokerage" + endpoint;
...
class AuthenticatedGetRequest : public GetRequest{
    ...
    public:
        AuthenticatedGetRequest(std::string endpoint, std::string url = "api.coinbase.com", std::string protocol = "https") : GetRequest(endpoint,url,protocol), m_url(url), m_endpoint(endpoint){}
...
class TradeAPI{
    private:
        deg::auth::Signer m_signer;
        GetRequest m_time_req = GetRequest("/time");
        AuthenticatedGetRequest m_order_req = AuthenticatedGetRequest("/orders/historical/batch");
        double m_last_rtt = 0;
    ...
    public:
        std::chrono::milliseconds get_server_time(){
            ...
            return std::chrono::milliseconds(std::stoll(document["epochMillis"].GetString()));
        }

        void get_orders(){
            std::cout << m_order_req.go() << std::endl;
        }
